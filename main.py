    ...
    for country in country_list:
        ...
        if id != 0:
            ...
            try:
                ...
                graph = gpt.ask(f"{info}\nНапиши новые показатели ВВП, численности, поддержки населения и военной мощи на основе этих данных. Они должны быть примерно такими же или немного меньше. Дай ответ в формате json. Пришли только json с новыми показателями без комментариев")
                    ...
                    if int(proj) <= int(year):
                        try:
                            ...
                            for new_country in country_list:
                                ...
                                if new_country[:-1] in text or random.randrange(0, 20) == 0:
                                    country_data = country_list[new_country]
                                else:
                                    continue
                                if country_data["id"] != 0:
